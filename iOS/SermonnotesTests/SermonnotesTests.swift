import XCTest
@testable import Sermonnotes

@MainActor
final class SermonnotesTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(SermonEntry(title: "New One"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(SermonEntry(title: "ToDelete"))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.entries = (0..<Store.freeLimit).map { SermonEntry(title: "Item \($0)") }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(SermonEntry(title: "Original"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        store.add(SermonEntry(title: "A"))
        store.add(SermonEntry(title: "B"))
